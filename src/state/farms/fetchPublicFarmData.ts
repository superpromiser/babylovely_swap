...
type PublicFarmData = {
  ...
  depositFee: string
  withdrawFee: string
  blovelyPerBlock: SerializedBigNumber
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const [info, totalAllocPoint, blovelyPerBlock, totalStakedAmount] =
  pid || pid === 0
  ...
  if (token.symbol === "BLOVELY" && isTokenOnly)
  ...
  const BlovelyPerBlock = blovelyPerBlock ? new BigNumber(blovelyPerBlock).div(BIG_TEN.pow(9)) : BIG_ZERO
  ...
  return {
    ...
    blovelyPerBlock: new BigNumber(BlovelyPerBlock).toJSON(),   
  }
}
